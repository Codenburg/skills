...
```bash
# Runtime + peer deps (NativeWind v5 + react-native-css + reanimated + safe-area)
npx expo install nativewind@preview react-native-css react-native-reanimated react-native-safe-area-context

# Tailwind CSS v4.1+ + PostCSS as dev dependencies
npx expo install --dev tailwindcss @tailwindcss/postcss postcss
```

> `react-native-reanimated` and `react-native-safe-area-context` are required peer dependencies in NativeWind v5. Without them the CSS runtime and animations will not work.
...
- postcss is bundled with Expo by default but is now listed explicitly as a dev dep in the official install command
...
> The fourth import (`@import "nativewind/theme"`) is required in NativeWind v5. It loads NativeWind-specific tokens such as elevation scales and platform variants.

## Two ways to enable `className` on React Native primitives

NativeWind v5 supports two ways to make `className` work on React Native components. Pick ONE per project.

### Approach A — Import rewrite (modern, recommended for new projects)

Import primitives from `react-native-css/react-native` instead of `react-native`. The CSS runtime rewrites the imports so `className` works directly — no wrapper component needed:

```tsx
import { View, Text, ScrollView, Pressable, TextInput } from "react-native-css/react-native";
...
This is the pattern used in the official NativeWind v5 docs. Use it when starting a new project or when you control the imports.

### Approach B — `useCssElement` wrapper (legacy, valid alternative)

Use this when you cannot change existing imports (e.g. components imported as `from "react-native"` elsewhere, or `Animated.ScrollView` / `Animated.View` from reanimated which the import rewrite does not cover). Full wrapper code is in the next section.
